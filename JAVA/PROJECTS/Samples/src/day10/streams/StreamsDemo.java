...
public class StreamsDemo {
	public static void main(String[] args) {
		...
		Map<Integer, String> resultNames = hmap.entrySet().stream().filter(map -> map.getKey().intValue() <= 22)
		...
		Map<Integer, String> resultFruits = hmapFruits.entrySet().stream()
		...
		Map<Integer, String> resultLetters = hmapLetters.entrySet().stream().filter(p -> p.getKey().intValue() <= 2) // filter  by
		...
		List<String> resultNotNulls = list.stream().filter(str -> str != null).collect(Collectors.toList());
